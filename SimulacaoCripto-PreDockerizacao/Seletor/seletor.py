...
def process_job(job):
    ...
    if sincronismo_relogios(validadores_que_sairam_da_fila) == False: #sincroniza os validadores coletados utilizando o relogio do banco com relogio de christian
        ...
        enviar_log_banco(f"Não existem validadores suficientes para continuar a eleição.", relogio_atual)#para continuar com a validação
        return
    else:
        tempo_2_stop = time()
...
def cadastrar_seletor_banco(seletor_atual):
    while True: 
        try:
            response = rq.post(f"http://127.0.0.1:5000/seletor/{seletor_atual.nome}/{float(seletor_atual.qtdMoeda)}/{seletor_atual.ip}")
            if response.status_code == 200:
...
def conectar_seletor_banco(seletor_atual):
    while True: 
        try:
            response = rq.post(f"http://127.0.0.1:5000/seletor/{seletor_atual.id}/{seletor_atual.ip}")
            if response.status_code == 200:
                print(f"Seletor cadastrado no banco")
                resposta = response.json()
...
with app.app_context():
    ...
    if not os.path.exists("session.txt"):
        ...
        seletor_atual = Seletor(nome="default", ip=ip)#default pois já existe nome nó banco atrelado ao id do seletor
